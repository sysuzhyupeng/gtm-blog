...
```
这时在点击之后，ga就会向统计后台服务器发送一个点击事件的请求。
其他相关的ga用法就不举例了，`因为本文的重点是gtm`。
 ...
 * ga代码一定要由前端人员维护。
 * 当想修改ga代码的时候，要走一遍发布上线流程(因为修改到前端代码)，更新效率低。
...
gtm脚本
-----
gtm的基础脚本由运营这里提供(主要是提供gtm的id)，类似于以下的脚本，要嵌入到页面代码中
```javascript
...
现在只要我们页面上有类名为btn的元素，就会不断触发统计了，可以在控制台中看到调用栈。到这里统计就完成了！
